...
class Grundy2RecBrute {
  ...
  boolean jouerGagnant(ArrayList<Integer> jeu) {
    ...
    if (jeu == null) {
      ...
      // mise en oeuvre de la règle numéro2 : Une situation (ou position) est dite gagnante pour la
      // machine (ou le joueur, peu importe), s'il existe AU MOINS UNE décomposition (c-à-d UNE
      // action qui consiste à décomposer un tas en 2 tas inégaux) perdante pour l'adversaire.
  ...
  boolean estPerdante(ArrayList<Integer> jeu) {
    ...
    if (jeu == null) {
      ...
      // si il n'y a plus que des tas de 1 ou 2 allumettes dans le plateau de jeu alors la situation
      // est forcément perdante (ret=true) = FIN de la récursivité
      if (!estPossible(jeu)) {
        ret = true;
      } else {
        // création d'un jeu d'essais qui va examiner toutes les décompositions possibles à partir
        // de jeu
        ArrayList<Integer> essai = new ArrayList<Integer>(); // size = 0
        // toute première décomposition : enlever 1 allumette au premier tas qui possède au moins 3
        // allumettes, ligne = -1 signifie qu'il n'y a plus de tas d'au moins 3 allumettes
        int ligne = premier(jeu, essai);
        while ((ligne != -1) && ret) {
          // mise en oeuvre de la règle numéro1 : Une situation (ou position) est dite perdante pour
          // la machine (ou le joueur, peu importe) si et seulement si TOUTES ses décompositions
          // possibles (c-à-d TOUTES les actions qui consistent à décomposer un tas en 2 tas
          // inégaux) sont TOUTES gagnantes pour l'adversaire. Si UNE SEULE décomposition (à partir
          // du jeu) est perdante (pour l'adversaire) alors la configuration n'EST PAS perdante. Ici
          // l'appel à "estPerdante" est RECURSIF.
          if (estPerdante(essai) == true) {
            ret = false;
          } else {
            // génère la configuration d'essai suivante (c'est-à-dire UNE décomposition possible) à
            // partir du jeu, si ligne = -1 il n'y a plus de décomposition possible
            ligne = suivant(jeu, essai, ligne);
          }
        }
      }
  ...
  void enlever(ArrayList<Integer> jeu, int ligne, int nb) {
    ...
    if (jeu == null) {
      ...
      // nouveau tas (case) ajouté au jeu (nécessairement en fin de tableau), ce nouveau tas
      // contient le nbre d'allumettes retirées (nb) du tas à séparer
      jeu.add(nb);
  ...
  int premier(ArrayList<Integer> jeu, ArrayList<Integer> jeuEssai) {
    ...
    if (jeu == null) {
      ...
      // sépare le tas (case numTas) en un tas d'UNE SEULE allumette à la fin du tableau, le tas en
      // case numTas a diminué d'une allumette (retrait d'une allumette), jeuEssai est le plateau de
      // jeu qui fait apparaître cette séparation
      if (numTas != -1) {
  ...
  int suivant(ArrayList<Integer> jeu, ArrayList<Integer> jeuEssai, int ligne) {
    ...
    if (jeu == null) {
      ...
      // si sur la même ligne (passée en paramètre) on peut encore retirer des allumettes, c-à-d si
      // l'écart entre le nombre d'allumettes sur cette ligne et le nombre d'allumettes en fin de
      // tableau est > 2, alors on retire encore 1 allumette sur cette ligne et on ajoute 1
      // allumette en dernière case
      if ((nbAllumEnLigne - nbAllDernCase) > 2) {
        ...
        // on recrée une nouvelle configuration d'essai identique au plateau de jeu, copie du jeu
        // dans JeuEssai
        jeuEssai.clear();
        ...
        // si il y a encore un tas et qu'il contient au moins 3 allumettes alors on effectue une
        // première séparation en enlevant 1 allumette
